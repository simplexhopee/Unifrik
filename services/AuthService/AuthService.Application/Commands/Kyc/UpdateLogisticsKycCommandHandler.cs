...
{
    public class UpdateLogisticsKycCommandHandler : IRequestHandler<LogisticsKycDto, string>
    {
        ...
        public UpdateLogisticsKycCommandHandler(IUserRepository repository, IUserServices userServices,
        ...
        public async Task<string> Handle(LogisticsKycDto request, CancellationToken cancellationToken)
        {
            ...
            user.BusinessLicensePath = $"{Guid.NewGuid()}{Path.GetExtension(request.BusinessLicensePath.FileName)}";
            await _fileService.UploadAsync(request.BusinessLicensePath.OpenReadStream(), user.BusinessLicensePath, request.BusinessLicensePath.ContentType);
